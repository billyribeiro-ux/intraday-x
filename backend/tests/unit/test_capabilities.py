...
def test_composite_union_and_deep_lookback() -> None:
    comp = CompositeProvider([(YFinanceProvider(), 10), (AlpacaProvider(), 5)])
    caps = comp.capabilities()
    # Union exposes Alpaca's extended history...
    assert caps.supports(Capability.EXTENDED_HISTORY_INTRADAY)
    # ...and the merged 1m lookback is the deeper of the two (Alpaca's).
